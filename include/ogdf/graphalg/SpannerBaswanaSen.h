		...
		for (int iteration = 1; iteration <= k-1; iteration++) {
			...
			for (node v : m_G.nodes) {
				...
				for (node center : m_G.nodes) {
					...
					if (A[center] != nullptr && (minEdge == nullptr || minEdge == A[center] || m_eps.less(weight(A[center]), minWeight))) {
...
public:
	SpannerBaswanaSenIterated(): SpannerIteratedWrapper<TWeight>(new SpannerBaswanaSen<TWeight>(), 1000) {};
};
