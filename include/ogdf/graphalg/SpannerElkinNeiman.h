...
public:
	SpannerElkinNeiman(): SpannerModule<TWeight>() {
		setEpsilon(DEFAULT_EPSILON);
	}

	SpannerElkinNeiman(double epsilon): SpannerModule<TWeight>() {
		setEpsilon(epsilon);
	}
...
private:
	...
	virtual typename SpannerModule<TWeight>::ReturnType execute() override {
		...
		for (node x : m_G->nodes) {
			...
			for (node u : m_G->nodes) {
				if (values[u] != std::numeric_limits<double>::lowest() && m_eps.geq(values[u], maxValue)) {
					edge e = firstEdge[u];
					if (!(*m_inSpanner)[e]) {
						m_spanner->newEdge(e);
						(*m_inSpanner)[e] = true;
...
public:
	SpannerElkinNeimanIterated(): SpannerIteratedWrapper<TWeight>(new SpannerElkinNeiman<TWeight>(), 200) {};
};
