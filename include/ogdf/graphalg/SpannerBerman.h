...
private:
	...
	virtual void init(const GraphAttributes& GA,
			  double stretch,
	 ...
	 */
	void inArborescence(const GraphAttributes& GA,
						node root,
	 ...
	 */
	void outArborescence(const GraphAttributes& GA,
						node root,
						NodeArray<edge>& predecessor,
						NodeArray<TWeight>& distance) {
		Dijkstra<TWeight> dijkstra;
	...
	void printStats(bool assert = false) {
		logger.lout() << "\nStats:" << std::endl;
		logger.lout() << m_spanner->numberOfEdges() << " covered of " << m_G->numberOfEdges() << std::endl;
		...
		if (assert) {
			OGDF_ASSERT((E1amountThick + E2amountThin + E1amountThinNotInE2 +
						 E2amountThickNotInE1) == m_spanner->numberOfEdges());
		}
#endif
	...
	bool _isThickEdge(edge e) {
		...
		for (node n : m_G->nodes) {
			...
			if (sd == std::numeric_limits<TWeight>::max() || td == std::numeric_limits<TWeight>::max()) {
				continue;
			}
	...
	void createAntispanner(const edge unsettledThinEdge,
		...
		for (edge e : m_G->edges) {
			...
			bool isInLocalSubgraph = (
				s_e != std::numeric_limits<TWeight>::max() &&
...
template<typename TWeight>
Logger SpannerBerman<TWeight>::logger(Logger::LogMode::Log, Logger::Level::High); // do not log by default
