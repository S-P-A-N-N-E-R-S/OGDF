...
public:
	 ...
	 */
	virtual ReturnType call(const GraphAttributes& GA,
					  double stretch,
					  GraphCopySimple& spanner,
					  EdgeArray<bool>& inSpanner) {
...
protected:
	 ...
	 */
	int64_t getTimeLeft() {
		return m_timelimit - getTimeNeeded();
	}
...
protected:
	struct TimeoutException {}; //!< A simple exception used to exit from the execution, if the timelimit is reached.
...
public:
	 ...
	 */
	static bool isMultiplicativeSpanner(const GraphAttributes& GA,
	 ...
	 */
	static void apspSpanner(const GraphAttributes& GA,
							 const GraphCopySimple& spanner,
...
template <typename TWeight>
void SpannerModule<TWeight>::apspSpanner(const GraphAttributes& GA,
										  const GraphCopySimple& spanner,
										  NodeArray<NodeArray<TWeight>>& shortestPathMatrix) {
