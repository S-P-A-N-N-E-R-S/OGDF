...
void test(
	const std::set<GraphProperty>& requirements,
	bool directed,
	bool weighted,
		...
		describe("stretch=" + to_string(stretch), [&]{
			forEachGraphItWorks(
			    ...
			    [&](const Graph& G) {
				    ...
				    TModule<TWeight> sm;
				    AssertThat(sm.call(GA, stretch, spanner, inSpanner), Equals(SpannerModule<TWeight>::ReturnType::Feasible) || Equals(SpannerModule<TWeight>::ReturnType::Optimal));

				    AssertThat(SpannerModule<TWeight>::isMultiplicativeSpanner(GA, spanner, stretch), IsTrue());
...
void testInvalidPreconditions(const GraphAttributes& GA, double stretch, const std::string& mustContain) {
...
void testMinimum(bool directed) {
    auto testInstance = [](const string& desc, const std::function<void(Graph&, GraphAttributes&, double&, int&, int&)>& createInstance) {
        auto getWeight = [](const GraphCopySimple& G, const GraphAttributes& GA) {
            int weight = 0;
            for (edge e : G.edges) {
                if (GA.has(GraphAttributes::edgeIntWeight)) {
                    weight += GA.intWeight(G.original(e));
                } else {
                    weight += 1;
                }
            }
        ...
        it("should be minimum on a " + desc, [&] {
            GraphCopySimple spanner;
            EdgeArray<bool> inSpanner;
            ...
            double stretch;
            int expectedNumEdges, expectedWeight;

            createInstance(G, GA, stretch, expectedNumEdges, expectedWeight);

            AssertThat(sm.call(GA, stretch, spanner, inSpanner), Equals(TModule<int>::ReturnType::Optimal));
            AssertThat(SpannerModule<int>::isMultiplicativeSpanner(GA, spanner, stretch), IsTrue());

            AssertThat(spanner.numberOfEdges(), Equals(expectedNumEdges));
            AssertThat(getWeight(spanner, GA), Equals(expectedWeight));
        });
    };

    testInstance("unweighted K4", [&](Graph& G,GraphAttributes& GA, double& stretch, int& expectedNumEdges, int& expectedWeight) {
        GA.directed() = directed;
        completeGraph(G, 4);
    ...
    testInstance("weighted K4", [&](Graph& G,GraphAttributes& GA, double& stretch, int& expectedNumEdges, int& expectedWeight) {
        GA.addAttributes(GraphAttributes::edgeIntWeight);
        GA.directed() = directed;
        Array<node> nodes;

        customGraph(G, 4, {}, nodes);
        GA.intWeight(G.newEdge(nodes[0], nodes[1])) = 5;
        GA.intWeight(G.newEdge(nodes[0], nodes[2])) = 6;
        GA.intWeight(G.newEdge(nodes[0], nodes[3])) = 4;
        GA.intWeight(G.newEdge(nodes[1], nodes[2])) = 3;
        GA.intWeight(G.newEdge(nodes[1], nodes[3])) = 10;
        GA.intWeight(G.newEdge(nodes[2], nodes[3])) = 2;
    ...
    testInstance("weighted cycle graph", [&](Graph& G,GraphAttributes& GA, double& stretch, int& expectedNumEdges, int& expectedWeight) {
        GA.addAttributes(GraphAttributes::edgeIntWeight);
        GA.directed() = directed;
        Array<node> nodes;

        customGraph(G, 6, {}, nodes);
        GA.intWeight(G.newEdge(nodes[0], nodes[1])) = 1;
        GA.intWeight(G.newEdge(nodes[1], nodes[2])) = 1;
        GA.intWeight(G.newEdge(nodes[2], nodes[3])) = 1;
        GA.intWeight(G.newEdge(nodes[3], nodes[4])) = 1;
        GA.intWeight(G.newEdge(nodes[4], nodes[5])) = 1;
        GA.intWeight(G.newEdge(nodes[5], nodes[0])) = 5;
...
void testTimelimit(bool directed, double stretch) {
	it("should timeout", [&] {
		...
		TModule sm;
		sm.setTimelimit(0);
		AssertThat(sm.call(GA, stretch, spanner, inSpanner), Equals(TModule::ReturnType::TimeoutInfeasible));
	});
}

template<template<typename> class TModule>
void testSpannerBerman(const std::set<GraphProperty>& requirements, const std::vector<double>& stretches) {
...
public:
	IteratedSpannerModule(
		typename SpannerModule<TWeight>::ReturnType returnType = SpannerModule<TWeight>::ReturnType::Feasible,
	...
	bool preconditionsCalled() {
		return m_preconditionsCalled;
	}
...
void testSpannerIteratedWrapper(
...
go_bandit([] {
	describe("IteratedWrapper", [] {
		testSpannerIteratedWrapper("propagates errors", [] (GraphAttributes& GA, GraphCopySimple& spanner, EdgeArray<bool>& inSpanner) {
			SpannerIteratedWrapper<int> wrapper(
				new IteratedSpannerModule<int>(SpannerModule<int>::ReturnType::Error, 1),
				10
			);
			AssertThat(wrapper.call(GA, 3.0, spanner, inSpanner), Equals(SpannerModule<int>::ReturnType::Error));
			AssertThat(wrapper.getExecutedIterations(), Equals(2));
		});
		testSpannerIteratedWrapper("stops after timeout", [] (GraphAttributes& GA, GraphCopySimple& spanner, EdgeArray<bool>& inSpanner) {
			SpannerIteratedWrapper<int> wrapper(
				new IteratedSpannerModule<int>(SpannerModule<int>::ReturnType::Feasible, 0, 0),
				10
			);
			AssertThat(wrapper.call(GA, 3.0, spanner, inSpanner), Equals(SpannerModule<int>::ReturnType::TimeoutInfeasible));
			AssertThat(wrapper.getExecutedIterations(), Equals(1));
		});
		testSpannerIteratedWrapper("returns at least one solution", [] (GraphAttributes& GA, GraphCopySimple& spanner, EdgeArray<bool>& inSpanner) {
			SpannerIteratedWrapper<int> wrapper(
				new IteratedSpannerModule<int>(SpannerModule<int>::ReturnType::Feasible, 9), // 9 failed iterations, the 10th one succeeds
				10
			);
			AssertThat(wrapper.call(GA, 3.0, spanner, inSpanner), Equals(SpannerModule<int>::ReturnType::Feasible));
			AssertThat(wrapper.getExecutedIterations(), Equals(10));
		});
		testSpannerIteratedWrapper("returns no feasible solution if every iteration is unfeasible", [] (GraphAttributes& GA, GraphCopySimple& spanner, EdgeArray<bool>& inSpanner) {
			SpannerIteratedWrapper<int> wrapper(
				new IteratedSpannerModule<int>(SpannerModule<int>::ReturnType::Feasible, 10),
				10
			);
			AssertThat(wrapper.call(GA, 3.0, spanner, inSpanner), Equals(SpannerModule<int>::ReturnType::NoFeasibleSolution));
			AssertThat(wrapper.getExecutedIterations(), Equals(10));
		});
		testSpannerIteratedWrapper("returns the best found solution", [] (GraphAttributes& GA, GraphCopySimple& spanner, EdgeArray<bool>& inSpanner) {
			SpannerIteratedWrapper<int> wrapper(
				new IteratedSpannerModule<int>(),
				6
			);
			AssertThat(wrapper.call(GA, 3.0, spanner, inSpanner), Equals(SpannerModule<int>::ReturnType::Feasible));
		...
		testSpannerIteratedWrapper("returns the best found solution with timeout", [] (GraphAttributes& GA, GraphCopySimple& spanner, EdgeArray<bool>& inSpanner) {
			SpannerIteratedWrapper<int> wrapper(
				new IteratedSpannerModule<int>(SpannerModule<int>::ReturnType::Feasible, 0, 4),
				6
			);
			AssertThat(wrapper.call(GA, 3.0, spanner, inSpanner), Equals(SpannerModule<int>::ReturnType::Feasible));
			AssertThat(wrapper.getExecutedIterations(), Equals(4));
			AssertThat(spanner.numberOfEdges(), Equals(8)); // 10-3+1 = 8, timeout is in 4th iteration
		});
	});
	describe("SpannerBaswanaSen", [] {
		describe("unweighted general graphs", [] {
			test<SpannerBaswanaSen, int>(
		...
		describe("Non-integer stretch", [] {
			testInvalidPreconditions<SpannerBaswanaSen<int>>(false, 1.2, "The stretch is required to be an integer");
		});
	...
	describe("SpannerKortsarzPeleg", [] {
		...
		describe("Directed graphs", [] {
			testInvalidPreconditions<SpannerKortsarzPeleg<int>>(true, 2, "The graph must be undirected");
		});
		describe("Graph with selfloops", [] {
			Graph G;
			GraphAttributes GA(G);
		...
		describe("Graph with int weighted edges", [] {
			...
			GA.directed() = false;
			emptyGraph(G, 1);
			testInvalidPreconditions<SpannerKortsarzPeleg<int>>(GA, 2.0, "The graph must be unweighted");
		});
	...
	describe("SpannerBasicGreedy", [] {
		...
		describe("double weighted general graphs", [] {
			test<SpannerBasicGreedy, double>(
				{GraphProperty::simple},
				false,
				true,
				{2.0, 2.5, 3.0, 3.5, 4.0}
		...
		describe("Directed graphs", [] {
			testInvalidPreconditions<SpannerBasicGreedy<int>>(true, 2, "The graph must be undirected");
		});
		testTimelimit<SpannerBasicGreedy<int>>(false, 1.0);
		testTimelimit<SpannerBasicGreedy<double>>(false, 1.0);
	});
	describe("SpannerBerman", [] {
		testSpannerBerman<SpannerBerman>({GraphProperty::simple, GraphProperty::connected}, {2.0, 2.5, 3.0, 3.5, 4.0});
	...
	describe("SpannerElkinNeiman", [] {
				...
				describe("stretch=" + to_string(stretch), [&]{
					forEachGraphItWorks(
						...
						[&](const Graph& G) {
							...
							int tries = 0;
							const int maxTries = 3; // This works in combination with the seed above
							// if this raises errors in the future, you can just increse the amount of
							// max tries.
							do {
								result = sm.call(GA, stretch, spanner, inSpanner);
								AssertThat(result, Is().EqualTo(SpannerModule<int>::ReturnType::Feasible)
									.Or().EqualTo(SpannerModule<int>::ReturnType::NoFeasibleSolution));
								tries++;
							} while (result != SpannerModule<int>::ReturnType::Feasible && tries < maxTries);
		...
		describe("Non-integer stretch", [] {
			testInvalidPreconditions<SpannerElkinNeiman<int>>(false, 1.2, "The stretch is required to be an integer");
		});
		describe("Directed graphs", [] {
			testInvalidPreconditions<SpannerElkinNeiman<int>>(true, 1, "The graph must be undirected");
		});
		describe("Graph with int weighted edges", [] {
			Graph G;
			GraphAttributes GA(G, GraphAttributes::edgeIntWeight);
			GA.directed() = false;
			emptyGraph(G, 1);
			testInvalidPreconditions<SpannerElkinNeiman<int>>(GA, 3.0, "The graph must be unweighted");
		});
		describe("Graph with double weighted edges", [] {
			Graph G;
			GraphAttributes GA(G, GraphAttributes::edgeDoubleWeight);
    ...
    describe("SpannerAhmed", [] {
        describe("unweighted general graphs", [] {
            test<SpannerAhmed, int>(
                    {GraphProperty::simple, GraphProperty::connected},
                    false,
                    false,
                    {2.0, 2.5, 3.0, 3.5, 4.0},
                    GraphSizes(5)
        ...
        describe("Directed graphs", [] {
            testInvalidPreconditions<SpannerAhmed<int>>(true, 2, "The graph must be undirected");
        });
        describe("Graph with multiple components", [] {
            Graph G;
            GraphAttributes GA(G);
            GA.directed() = false;
            emptyGraph(G, 2);
            testInvalidPreconditions<SpannerAhmed<int>>(GA, 2.0, "The graph is not connected");
        });
        describe("Minimum Spanner", [] {
            testMinimum<SpannerAhmed>(false);
        });
        testTimelimit<SpannerAhmed<int>>(false, 1.0);
        testTimelimit<SpannerAhmed<double>>(false, 1.0);
    });
});
